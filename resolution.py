...
data_folder = {180: 'cooking-180p', 360: 'cooking', 720: 'cooking-720p', 1080: 'cooking-1080p'}
right = {}
wrong = {}
right_all = {}
wrong_all = {}
...
for f in filelist:
    ...
    print f
    img = cv2.imread(f)
    rtn_msg, objects_truth = cc.process(img, [])
    for res in resolution_list:
        if res == 1080:
            continue
        img = cv2.imread(f.replace(data_folder[1080], data_folder[res]))
        rtn_msg, objects = cc.process(img, [])

        if objects == objects_truth:
            right_all[res] += 1
